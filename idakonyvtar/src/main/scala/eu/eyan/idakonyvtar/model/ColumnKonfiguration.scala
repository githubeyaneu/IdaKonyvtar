...
object ColumnKonfiguration {
  ...
  class Builder(columnCount: Int, rowCount: Int) {
    ...
    def build() = new ColumnKonfiguration(new Excel(columns, rows, table.toMap))
  }
}
...
class ColumnKonfiguration(val table: Excel) {

  //TODO remove and make only relevant methods
  def isTrue(columnName: String, columnConfiguration: ColumnConfigurationsValue) = getValue(columnName, columnConfiguration).equalsIgnoreCase("Igen")
...
//  }
  ...
  private def getValue(columnName: String, columnConfiguration: ColumnConfigurationsValue): String = {
    val columnIndex = getColumnIndex(columnName)
    val configurationIndex = getConfigurationIndex(columnConfiguration)

    if (columnIndex.index > 0 && configurationIndex.index > 0) table.getCell((configurationIndex,columnIndex)).content.get
    else ""
  }

  private def getColumnIndex(columnName: String) = table.rowFromFirstColumn(columnName).getOrElse(Row(-1))

  private def getConfigurationIndex(configurationName: ColumnConfigurationsValue) = {
    table.firstRowCells.filter(_.content.get.toLowerCase == configurationName.name.toLowerCase)(0).column
