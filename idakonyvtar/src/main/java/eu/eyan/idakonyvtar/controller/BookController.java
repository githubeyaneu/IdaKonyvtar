...
public class BookController implements
		IDialogController<BookControllerInput, Book> {
	...
	public String getTitle() {
		if (model.getColumns().indexOf("Szerző") >= 0) {
			return "Book adatainak szerkesztése - "
					+ model.getBook().getValue(
							model.getColumns().indexOf("Szerző"));
		} else {
			return "Book adatainak szerkesztése";
		}
	}
	...
	private void initFieldsActionBindings() {
		for (int columnIndex = 0; columnIndex < model.getColumns().size(); columnIndex++) {
			...
			boolean autoComplete = model.getColumnConfiguration().isTrue(
					columnName, ColumnConfigurations.AUTOCOMPLETE);
			boolean multi = model.getColumnConfiguration().isTrue(columnName,
					ColumnConfigurations.MULTIFIELD);
			if (autoComplete) {
				List<String> columnList = BookHelper.getColumnList(
						model.getBookList(), columnIndex);
				if (multi) {
					MultiFieldJComboBox mmcombo = (MultiFieldJComboBox) view
							.getEditors().get(columnIndex);
					mmcombo.setAutoCompleteList(columnList);
					multimezőBind(mmcombo, new BookFieldValueModel(columnIndex,
							model.getBook()));
				} else {
					JComboBox<?> comboBox = (JComboBox<?>) view.getEditors()
							.get(columnIndex);
					Bindings.bind(comboBox, new ComboBoxAdapter<String>(
							columnList, new BookFieldValueModel(columnIndex,
									model.getBook())));
					AutoCompleteDecorator.decorate(comboBox);
				}
			} else {
				if (multi) {
					MultiFieldJTextField mmc = (MultiFieldJTextField) view
							.getEditors().get(columnIndex);
					multimezőBind(mmc, new BookFieldValueModel(columnIndex,
							model.getBook()));
				} else {
					Bindings.bind(
							(JTextField) view.getEditors().get(columnIndex),
							new BookFieldValueModel(columnIndex, model
									.getBook()));
				}
			}
		}
	...
	private void multimezőBind(final MultiField<String, ?> mmc,
			final BookFieldValueModel bookFieldValueModel) {
		bookFieldValueModel.addValueChangeListener((
				PropertyChangeEvent propertyChangeEvent) -> {
			if (ObjectUtils.notEqual(propertyChangeEvent.getNewValue(),
					propertyChangeEvent.getOldValue())) {
				mmc.setValues(getMultiFieldList((String) propertyChangeEvent
						.getNewValue()));
			}
		});
		...
		mmc.addPropertyChangeListener((PropertyChangeEvent propertyChangeEvent) -> {
			bookFieldValueModel.setValue(Joiner.on(BookHelper.LISTA_SEPARATOR)
					.skipNulls().join(mmc.getValues()));
		});
	}

	private static List<String> getMultiFieldList(String value) {
		String[] strings = (value).split(BookHelper.LISTA_SEPARATOR_REGEX);
		List<String> list = newArrayList(strings).stream()
				.filter((String s) -> {
					return !s.isEmpty();
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							...
							try {
								...
								view.getIsbnSearchLabel().setText(
										"Nincs találat");
								view.getIsbnSearchLabel()
										.setIcon(
												new ImageIcon(
														Resources
																.getResource("icons/error.gif")));
							} finally {
								view.getEditors()
										.forEach(
												component -> component
	...
	private void prozessIsbnData(List<Marc> marcsToIsbn) {
		for (String column : model.getColumns()) {
			...
			try {
				marcCodesToColumns = model.getColumnConfiguration()
						.getMarcCodes(column);
				for (Marc marc : marcsToIsbn) {
					for (Marc columnMarc : marcCodesToColumns) {
						if (isMarcsApply(marc, columnMarc)) {
							columnValue += columnValue.equals("") ? marc
									.getValue() : ", " + marc.getValue();
						}
					}
				}
				model.getBook().setValue(model.getColumns().indexOf(column),
						columnValue);
			} catch (Exception e) {
				e.printStackTrace();
