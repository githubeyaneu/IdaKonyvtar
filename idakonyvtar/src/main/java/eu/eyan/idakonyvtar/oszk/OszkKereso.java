...
public class OszkKereso {
	public static String isbnKeresOszkban(final String isbn) throws IOException {
		...
		// Marc hosszú
		return HttpHelper.postUrl("http://nektar1.oszk.hu/LVbin/LibriVision/"
				+ fullMarcLink, "");
	}

	private static String findTextInUrl(final String host,
			final String postParameter, final String lineGrep,
			final String regex_prefix, final String regex,
			final String regexPost) throws IOException {
		String postUrl = HttpHelper.postUrl(host, postParameter);
		String line = Unix4j.fromString(postUrl).grep(lineGrep)
				.toStringResult().toString();
		Matcher matcher = Pattern.compile(regex_prefix + regex + regexPost)
				.matcher(line);
		matcher.find();
		String group = matcher.group();
		final String sessionId = group.substring(regex_prefix.length(),
				group.length() - regexPost.length()).replaceAll("&amp;", "&");
		return sessionId;
	}

	public static List<Marc> getMarcsToIsbn(String isbn)
			throws OszkKeresoException {
		...
		try {
			...
			Matcher marcTableMatcher = Pattern.compile(
					"<table class=\"record\">.*?</table>").matcher(source);
			marcTableMatcher.find();
			String marcTable = marcTableMatcher.group();
			// System.out.println(marcTable);

			Matcher marcSorMatcher = Pattern.compile("<tr.*?</tr").matcher(
					marcTable);
