...
public class ExcelHandler
{
    ...
    public static void saveLibrary(File targetFile, Library library)
    {
        ...
        try
        {
            WritableWorkbook workbook = Workbook.createWorkbook(targetFile, getWorkbookSettings());
            WritableSheet writablesheet1 = workbook.createSheet(new String(BOOKS), 0);
            ...
            WritableSheet writablesheet2 = workbook.createSheet(COLUMN_CONFIGURATION, 1);
    ...
    {
        String sourceLibrary = FilenameUtils.getFullPath(fileToSave.getAbsolutePath());
        String sourceFileName = FilenameUtils.getName(fileToSave.getAbsolutePath());
        File backupLibrary = new File(sourceLibrary + "backup");
        backupLibrary.mkdirs();
        File backupFile = new File(backupLibrary.getAbsoluteFile() + File.separator + sourceFileName + "_backup_" + new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date()) + ".zip");
