...
public class BookEditorTest extends AbstractUiTest
{
    ...
    public void setUp()
    {
        ...
        Book book = new Book.Builder(columns.size())
                .withValue(0, "Érték1")
                .build();
        bookController = new BookController();
        BookControllerInput bookControllerInput = new BookControllerInput.Builder()
                .withBook(book)
                .withColumns(columns)
                .withColumnConfiguration(new ColumnKonfiguration.Builder(3, columns.size() + 1)
                        .withRow("", MULTIFIELD.getName(), AUTOCOMPLETE.getName())
                        .withRow(columns.get(0), "", "")
                        .withRow(columns.get(1), "", "igen")
                        .withRow(columns.get(2), "igen", "")
                        .withRow(columns.get(3), "igen", "igen")
                        .build())
                .withBookList(newArrayList(
                        book
    ...
    public void testMultiFieldAutoComplete()
    {
        ...
        bookEditor.clickSave();
        assertThat(bookController.getOutput().getValue(3)).isEqualTo("b + c + abc");
    }
